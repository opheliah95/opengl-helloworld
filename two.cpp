...
const char *fragShaderSource = "#version 330 core\n"
                               ...
                               "void main() {"
                               "  frag_colour = vec4(0.3, 0.0, 0.5, 1.0);"
                               "}";
...
int main()
{
    ...
    // step1: vertex processing -> how to access actual vertex data
    GLuint VAO[2], VBO[2];
    // generate VAO and binds them
    glGenVertexArrays(2, VAO);
    // generate VBO and associate VBO to VAO
    glGenBuffers(2, VBO);
    for (unsigned int i = 0; i < sizeof(VAO) / sizeof(VAO[0]); i++)
    {
        glBindVertexArray(VAO[i]);
        // add array buffer
        glBindBuffer(GL_ARRAY_BUFFER, VBO[i]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(points[i]), points[i], GL_STATIC_DRAW);
        //create attribute associated to data
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 3, (void *)0);
        glEnableVertexAttribArray(0);
        std::cout << points[i][0] << std::endl;
    }

    // create vertex and frag shader
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindVertexArray(VAO[0]); // binds VAO
        glDrawArrays(GL_TRIANGLES, 0, 3);
        glBindVertexArray(VAO[1]); // binds VAO
        glDrawArrays(GL_TRIANGLES, 0, 3);
